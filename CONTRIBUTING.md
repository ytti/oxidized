...
if you need help, but you can also help other users by reviewing their issues and commenting them.
...
Content contributions are always welcome. You do not need to be a maintainter for this.
You even do not need to be a ruby programmer, as an example, the documentation always needs some
enhancements :-)

Contributions can be submited through pull requests in github. For a full explaination how to
contribute some content, see [How to contribute content](#how-to-contribute-content).

## Help Needed

As things stand right now, `oxidized` is maintained by a few persons. A great
...
maintaining the infrastructure code ([Oxidided maintainters](#oxidized-maintainters))
...
favorite hardware and a technical background which ich plenty sufficient to support
...
A model maintainer has folowing tasks:
...
* Note that Oxidized has a very liberal approch to changes: if someone needed it,
I need good reason to reject it.
* When a model maintainer comments a Pull Request as ready to merge into main,
an [Oxidided maintainter](#oxidized-maintainters) will merge it without any
further checks. If nothing happens, the model maintainer should trigger an Oxidized maintainer directly ;-).
* If there are mutliple model maintainers for a model, they should agree among themselves
...
The modem maintainers are listed in [docs/Supported.OS-Types.md]. Add or remove yourself in the table and push the change to github (pull request). Don't know how? Have a look at [How to contribute content](#how-to-contribute-content).
...
### Create a new branch
Create a new brach for your contribution. Choose a concise name for your branch. If your contribution refers to an issue, you may prepend the number of the issue to the branch name.
```bash
git checkout -b 1234-your-branch-name
```

### Create a ruby bundle
You need [Bundler](https://bundler.io/) to install ruby dependencies localy. If it is not already
...
If you don't want to use the configuration under `~/.config/oxidized/`, you can set the path to your specific configuration whith the environement-variable `OXIDIED_HOME': `export OXIDIZED_HOME=~/oxidized-config/`.
...
### Commit your work
You can save your changes anytime to the branch. These changes are saved localy and not pushed upstream:
```bash
git status
git diff
...
The commits will be seen in the pull request, so be concise an remember that someone will try to
...
```bash
push -u origin 1234-your-branch-name
```
...
## One last word
Have fun, and don't forget to graturlate yourself for your great contribution to [Oxidized](https://github.com/ytti/oxidized/)!
