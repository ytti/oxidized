module Oxidized
  class Input
    module CLI
      ...
      def connect_cli
        logger.debug "lib/oxidized/input/cli.rb: Running post_login commands at #{node.name}"
        @post_login.each do |command, block|
          logger.debug "lib/oxidized/input/cli.rb: Running post_login command: #{command.inspect}, block: #{block.inspect} at #{node.name}"
          block ? block.call : (cmd command)
        end
      end

      def disconnect_cli
        logger.debug "lib/oxidized/input/cli.rb Running pre_logout commands at #{node.name}"
