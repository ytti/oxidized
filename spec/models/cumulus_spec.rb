...
describe 'Model Cumulus' do
  ...
  it 'runs on MSN2010 with Cumulus Linux 5.9.2 (nvue mode)' do
    ...
    model = YAML.load_file('spec/models/data/cumulus#MSN2010_5.9.2_nvue#custom_simulation.yaml')
  ...
  it 'runs on VX with Cumulus Linux 5.4.0 (frr mode)' do
    ...
    model = YAML.load_file('spec/models/data/cumulus#VX_5.4.0_frr#custom_simulation.yaml')
    mockmodel = MockSsh.new(model)
    Net::SSH.stubs(:start).returns mockmodel
