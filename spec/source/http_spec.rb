...
describe Oxidized::HTTP do
  ...
  describe "#string_navigate" do
    ...
    it "should be able to navigate multilevel-hash" do
      ...
      http.string_navigate(h1, "jotain.2").must_equal "jotain"
    end
    it "should be able to navigate hash/array combination" do
      Oxidized::HTTP.new.string_navigate(h1, "jotain.2").must_equal "jotain"
    end
    it "should be able to navigate multilevel-hash" do
      Oxidized::HTTP.new.string_navigate(h1, "inventory[0].ip").must_equal "10.10.10.10"
    end
    it "should return nil on non-existing string key" do
      Oxidized::HTTP.new.string_navigate(h1, "jotain.3").must_equal nil
    end
    it "should return nil on non-existing array index" do
      Oxidized::HTTP.new.string_navigate(h1, "inventory[3]").must_equal nil
    end
  end
end
